...
{
    public class BlackBoxCO2Calibration : DialogFragment
    {
        ...
        public BlackBoxCO2Calibration(EBlackBoxesBlackBox blackbox, ERoom room, Action onResume)
        {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            string info = "<html><body text='white' style='text-align:justify;'>" + Resources.GetText(Resource.String.co2_calibration_info) + "</body></html>";
            ...
            string wait = "<html><body text='white' style='text-align:justify;'>" + Resources.GetText(Resource.String.co2_calibration_wait) + "</body></html>";
        ...
        public async override void OnResume()
        {
            ...
            mqttClient.Client.UseConnectedHandler(async e =>
            {
                ...
                try
                {
                    await mqttClient.SubscribeAsync($"blackbox/{SelectedRoom.center.idCenter}/{SelectedRoom.type.idRoomType}/{SelectedRoom.number}/0/{SelectedBlackBox.mac}/calibration/co2zero");
                    Log.Info("MQTT", "SUBSCRIBED TO CO2 ZERO");
                }
                catch (Exception ex)
                {
            ...
            mqttClient.Client.UseApplicationMessageReceivedHandler(async e =>
            {
                ...
                switch (topic.Last())
                {
                    //FINALIZADO LA FASE CO2 CERO
                    case "co2zero":
                        Log.Info("MQTT", "MESSAGE RECEIVED CO2 ZERO");
                        Activity.RunOnUiThread(() =>
                        {
        ...
        private async void InfoContinue_Click(object sender, EventArgs e)
        {
            if (mqttClient != null && mqttClient.Client.IsConnected)
            {
                try
                {
                    await mqttClient.PublishAsync($"blackbox/{SelectedRoom.center.idCenter}/{SelectedRoom.type.idRoomType}/{SelectedRoom.number}/0/{SelectedBlackBox.mac}/set-co2zero", "on", false);
                    Log.Info("MQTT", "MESSAGE SENT CO2 ZERO");
